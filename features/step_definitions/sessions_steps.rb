Given /^I have "([^"]*)" college in "([^"]*)"$/ do |college, conference|
  c = Conference.create!(:name => conference) unless c = Conference.where(:name => conference).first
...
Given /^I have confirmed user "([^\"]*)" "([^\"]*)" with username "([^\"]*)", email "([^\"]*)", password "([^\"]*)", college "([^\"]*)" and affiliation "([^\"]*)"$/ do |first_name, last_name, username, email, password, college, affiliation|
