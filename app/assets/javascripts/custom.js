...
$(function() {
    $(".smack").click(function() {
        jQuery.facebox("<span style='text-align: center;padding-left:45%'><img src='/assets/ajax-loader.gif' /></span>")
    });
});
...
function formatTitle() {
    return '<div id="photo-close"><span><a href="javascript:;" onclick="$.fancybox.close();"><img src="/assets/fancybox/fancy_close.png" /></a></span></div>';
}
$(function() {
...
$(document).ready(function() {
    clientSideValidations.callbacks.element.pass = function(element, callback) {
        $(element).parents('.field_with_errors').each(function() {
            var description = $(this).parent('.entry').siblings('.description');
            var description_html = description.find('.field_with_errors').html();
            var element_html = $(this).html();
            var parent = $(this).parent();
            var text = parent.find('input').val();
            $(this).remove();
            parent.append(element_html);
            parent.find('.message').remove();
            parent.find('input').val(text);
            if (description.length > 0) {
                description.find('.field_with_errors').remove()
                description.append(description_html);
                description.find('.message').remove();
            }
        });
    }
});
