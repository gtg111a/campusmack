$(document).ready(function() {
    ...
    $("#search_contacts").live('submit', function() {
        indicator_contacts();
        $.get($("#search_contacts").attr("action"), $("#search_contacts").serialize(), null, "script");
        return false;
    });

    $("#pagination_container a").live("click", function(event) {
    ...
    $('.clear_contacts').click(function(event) {
        event.preventDefault();
        $('#search').val(null);
        indicator_contacts();
        $.get($("#search_contacts").attr("action"), null, null, "script");
    });
    ...
    $('#remove_selected').live('click', function(event) {
        ...
        $.ajax({
            ...
            data: $("#contact_list input").serialize() + ('&group_id=' + $(this).attr('rel').replace('remove', ''))
        });
...
function reload() {
    if ($("#rowclick1").children('tr').length == $(".chk_box:checked").length) {
        var delete_all = true;
    }
    indicator_contacts();
    if ($('#pagination_container em').length > 0) {
        if ($('#pagination_container .next').length == 0 & delete_all == true) {
            var prev = $('#pagination_container em').text() - 1;
            $.get($("#search_contacts").attr("action"), $("#search_contacts").serialize() + ("&page=" + prev), null, "script");
        }
        else {
            $.get($("#search_contacts").attr("action"), $("#search_contacts").serialize() + ("&page=" + $('#pagination_container em').text()), null, "script");
        }
    }
    else {
        $.get($("#search_contacts").attr("action"), $("#search_contacts").serialize(), null, "script");
...
function reload_contacts(id, href, after_load) {
    ...
    if (height_contacts > height_groups) {
        $(id).html("<table><td class='ajax_loader' style='height:" + height_rowclick + "px;'><img src='/assets/ajax-loader.gif'/></td></table>");
    }
    else {
        $(id).html("<table><td class='ajax_loader' style='height:" + height_groups + "px;'><img src='/assets/ajax-loader.gif'/></td></table>");
    }
...
function reload_contacts_groups() {
    ...
    if (height_contacts < height_groups) {
        $('.contact_groups').replaceWith("<ul class='contact_groups' style='background:none;'><table style='width: 136px;'><td class='ajax_loader' style='height:" + height_groups + "px;'><img src='/assets/ajax-loader.gif'/></td></table></ul>");
    }
    else {
        $('.contact_groups').replaceWith("<ul class='contact_groups' style='background:none;'><table><td class='ajax_loader' style='height:" + height_contacts + "px;'><img src='/assets/ajax-loader.gif'/></td></table></ul>");
    }
...
function indicator_progress_in_facebox(href) {
    ...
    $('#facebox .popup .content').html("<table style='text-align:center;width:100%;'><td class='ajax_loader' style='vertical-align:middle;height:" + height + "px;'><img src='/assets/ajax-loader.gif'/></td></table>");
    return false;
}

function reload_title() {
    var h = $('.content_title h1').height();
    $('.content_title h1').html("<table><td class='ajax_loader' style='height:" + h + "px;'><img src='/assets/ajax-loader.gif'/></td></table>");
...
function reload_pagination(href) {
    ...
    $('#pagination_container div.pagination').html("<table><td class='ajax_loader' style='height:" + height + "px;'><img src='/assets/ajax-cb-loader.gif'/></td></table>");
    $('#pagination_container').load(href + ' ' + '#pagination_container > *');
}

function indicator_pagination(href) {
    if (href) {
    ...
    var height = $('#pagination_container .pagination').height();
    $('#pagination_container div.pagination').html("<table><td class='ajax_loader' style='height:" + height + "px;'><img src='/assets/ajax-cb-loader.gif'/></td></table>");
}

function indicator_contacts(id, href, after_load) {
    if (id) {
